...
namespace SimpleCompiler
{
    public class Function : JackProgramElement
    {
        ...
        // need to check for each identifier that the function/global conatins him 
        ...
        public override void Parse(TokensStack sTokens)
        {
            ...
            if(Name=="main")
            {
                if(((VarType)tType).Name!="int")
                    throw new SyntaxErrorException("Expected int as main return type, received " + tType, tType);
            }

            //After the name there should be opening paranthesis for the arguments
            Token t = sTokens.Pop(); //(
            if(!(t is Parentheses) || ((Parentheses)t).Name!='(')
                throw new SyntaxErrorException("Expected function name, received " + t, t);
            ...
            //TODO Dont slash this lines
            if(Body.Count>=1)
            { 
            if( !(((Body[Body.Count-1])) is ReturnStatement ) )
                throw new SyntaxErrorException("Expected function name, received " , null); }
            //Finally, the function should end with }
            Token tEnd = sTokens.Pop();//}
            if(!(tEnd is Parentheses) || !(((Parentheses)tEnd).Name=='}'))
                throw new SyntaxErrorException("Expected function name, received " + tEnd, tEnd);
        }
